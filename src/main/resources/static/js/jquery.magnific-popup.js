	...
	proto: {
		...
		getIframe: function(item, template) {
			...
			$.each(iframeSt.patterns, function() {
				if(embedSrc.indexOf( this.StudentIndex ) > -1) {
					if(this.id) {
	...
	proto: {
		initGallery: function() {
			...
			_mfpOn('UpdateStatus'+ns, function(e, data) {
				if(data.text) {
					data.text = _replaceCurrTotal(data.text, mfp.currItem.StudentIndex, mfp.items.length);
				}
			});

			_mfpOn(MARKUP_PARSE_EVENT+ns, function(e, element, values, item) {
				var l = mfp.items.length;
				values.counter = l > 1 ? _replaceCurrTotal(gSt.tCounter, item.StudentIndex, l) : '';
			});
