...
function getProject(user_id){
	$.ajax({
        ...
        success:function(res){
			...
			stu_table.innerHTML="<tr style='border: none;background-color: grey'><th style='width: 100px'>学生名</th><th style='width: 200px'>账号</th><th style='width: 100px'>学校</th><th style='width: 300px'>操作</th><th style='width: 300px'>评分</th></tr>";
			for(var i=0;i<res.data.length;i++){
				var data = res.data[i];
				stu_table.innerHTML+="<tr style='border: none'><td><label>"+data.stu_name+"</label></td><td><label>"+data.account+"</label></td><td><label>"+data.stu_name+"</label></td><td><a href='TeacherCheckStudentInfo?stu_id="+data.stu_id+"'>查看</a>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<a href='TeacherCheckLog?stu_id="+data.stu_id+"'>日志</a></td><td><input id='score"+data.stu_id+"' value='"+data.stu_score+"'type='number' max='100' min='0' style='width: 80px;margin-left: 20px'><button class='studentinput bu' style='width: 80px;text-align: center;padding-left: 10px;margin-left: 50px' value="+data.stu_id+">提交</button></td></tr>";
			}
